...
if uploaded_file:
    ...
    try:
        ...
        # ✅ Evaluation
        actual = ts[target].iloc[-30:] if len(ts) >= 30 else ts[target]
        ...
        st.write(f"**ARIMA Performance:** MAE = {mae:.2f}, RMSE = {rmse:.2f}")
    ...
    try:
        ...
        forecast_values = forecast_p["yhat"].iloc[-30:]

        mae_p = mean_absolute_error(actual, forecast_values)
        rmse_p = np.sqrt(mean_squared_error(actual, forecast_values))

        st.write(f"**Prophet Performance:** MAE = {mae_p:.2f}, RMSE = {rmse_p:.2f}")
